...
int hdev;
struct uloop_fd fd;
struct btle_config config = {
	.lescan_timeout = 15000,
	.lescan_max_age = 60,
	._6lowpan = 1,
};

static void btle_connect(struct btle_peer *peer)
{
	bdaddr_t bdaddr;
...
static void btle_cb(struct uloop_fd *fd, unsigned int events)
{
	int len;
	...
	case EVT_LE_META_EVENT:
		btle_le_meta_evt(len);
		break;
	}
}

static void btle_open(void)
{
...
static void btle_lescan(int enable)
{
	hci_le_set_scan_parameters(fd.fd, 1, 16, 16, 0, 0, 1000);
	hci_le_set_scan_enable(fd.fd, enable, 1, 1000);
}

static void btle_lescan_abort_cb(struct uloop_timeout *t)
{
	struct btle_peer *peer, *tmp;
	struct timespec ts;
...
static void btle_lescan_timer_cb(struct uloop_timeout *t)
{
...
{
	lescan_abort.cb = btle_lescan_abort_cb;
	lescan_timer.cb = btle_lescan_timer_cb;
